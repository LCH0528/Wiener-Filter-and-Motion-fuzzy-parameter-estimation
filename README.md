...
(2) len_search.m: Blur length estimation. The blur length is estimated primarily using the inverse spectrum method.

(3) C_g_show.m: Characteristics of the inverse spectrum of motion-blurred images. In the code, a grayscale image of size 101101 is selected (with a white region of size 33 and grayscale value 255 in the center, and all other regions having a grayscale value of 0). The corresponding inverse spectrum image is represented in three dimensions to illustrate the characteristics of the inverse spectrum of motion-blurred images.

(4) error_analysis.m: Generates error curves for blur length and blur angle, and performs error analysis by observing the curves. Note: Before executing the blur parameter estimation function, the statements related to plotting within the function must be commented out.

(5) K-Value Estimation: For an ideal motion-blurred image, by comparing the four curves—MSE-K, SNR-K, PSNR-K, and ISNR-K—MSE is ultimately selected as the metric for estimating the K-value. A smaller MSE indicates better restoration quality, and the corresponding K-value can be used as the estimated K-value, i.e., the optimal K-value. For actual motion-blurred images, the restoration performance is evaluated using objective metrics without a reference image: H (entropy), GMG (mean gray-level gradient), and SF (spatial frequency). Code files: K_blurred_search.m: Compares the grid method (coarse search) and the particle swarm optimization algorithm for searching the K value, and plots the fitness curve of the particle swarm optimization algorithm. K_search.m: Estimates the K value using the particle swarm optimization algorithm (includes evaluation metrics without reference images). MSE_search.m: Fitness (MSE). Note: In the code, the grid method and the particle swarm optimization algorithm are used to search for the minimum MSE and the corresponding optimal K value, respectively. Here, MSE or H also serves as the fitness function for the particle swarm optimization algorithm.

(6) Objective evaluation metrics without reference images. calculate_GMG.m: Average Gray-Scale Gradient (GMG). calculate_SF.m: Spatial Frequency (SF). Note: MATLAB includes a built-in function `entropy` for calculating image entropy (H).

(7) wiener_filter_best.m: Function file used to estimate blur parameters: blur angle, blur length, and K-value.

(8) WIENER_Processing.m: Parameter estimation and Wiener filtering for an ideal motion-blurred image.

(9) Parameter estimation and Wiener filtering for an actual motion-blurred image. WIENER_Processing_realimage.m: Parameter estimation and Wiener filtering for an actual motion-blurred image. WIENER_Processing_realimage2.m: Unlike the parameter estimation method described above, this method first estimates the blur angle using the previous method, then subjectively compares the restored images obtained with different blur lengths and K-values, and selects the blur length and K-value corresponding to the image with the best restoration results as the estimated values, before finally performing Wiener filtering.
